...
def detect_aruco(frame):
    ...
    if len(corners) > 0:
        ...
        for (markerCorner, markerId) in zip(corners, ids):
            ...
            arucoCenterX = int((tL[0] + bR[0]) / 2.0)
            arucoCenterY = int((tL[1] + bR[1]) / 2.0)
        returnVal = (globalCorners, tR, bR, bL, tL, frameCenter)
    return returnVal
...
def newOffsetFromCenter(topRight, bottomRight, bottomLeft, topLeft, frameCenter):
    pass
...
def get_offset_from_center(topRight, bottomRight, bottomLeft, topLeft, frameCenter):
    ...
    #print(f"trx: {trX}, trY: {trY}, tlX: {tlX}, tlY: {tlY}, brX: {brX}, brY: {brY}, blX: {blX}, blY: {blY}")

    #THEY SHOULD ALL BE IN ONE CONDITIONAL STATEMENT, NOT MULTIPLE IF STATEMENTS
    #CONNECT THE HORIZONTAL AND THE VERTICAL TOGETHER
    ...
    if (tlX < frameCenter[0]) and (tlY < frameCenter[1]) and (trX > frameCenter[0]) and (trY < frameCenter[1]) and (blX < frameCenter[0]) and (blY > frameCenter[1]) and (brX > frameCenter[0]) and (brY > frameCenter[1]):
        ...
        elif trX < frameCenter[0] and brX < frameCenter[0]:
        ...
        #if above center
        elif brY < frameCenter[1] and blY < frameCenter[1]:
